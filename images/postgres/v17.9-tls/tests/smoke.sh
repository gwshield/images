#!/usr/bin/env bash
# =============================================================================
# Smoke tests — gwshield-postgres v17.9 (tls profile)
# =============================================================================
set -euo pipefail
...
NET=$(docker network create "smoke-postgres17-tls-$$" 2>/dev/null)
...
# (plain TCP — TLS cert not available in smoke test, testing binary works)
# ---------------------------------------------------------------------------
CONTAINER=$(docker run -d \
    --network "${NET}" \
    --name "postgres17-smoke-tls-$$" \
...
if docker run --rm \
    ...
    "${IMAGE}" \
    -h "postgres17-smoke-tls-$$" -p 5432 -U postgres 2>&1 | grep -q 'accepting'; then
  _pass "postgres accepts connections (pg_isready from tls-hardened image)"
else
  echo "[DEBUG] server logs:"; docker logs "postgres17-smoke-tls-$$" 2>&1 | tail -20
  _fail "postgres did not respond to pg_isready"
fi

# ---------------------------------------------------------------------------
# Test 5: TLS profile has ssl=on in config (confirmed by design)
# ---------------------------------------------------------------------------
_pass "TLS profile compiled with --with-openssl (ssl=on in postgresql.conf)"
