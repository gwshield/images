#!/usr/bin/env bash
# =============================================================================
# Smoke tests — gwshield-postgres v17.9-timescale (time-series profile)
# Extensions: pg_partman, pg_cron, TimescaleDB
# =============================================================================
set -euo pipefail
...
NET=$(docker network create "smoke-postgres17-timescale-$$" 2>/dev/null)
...
CONTAINER=$(docker run -d \
    --network "${NET}" \
    --name "postgres17-smoke-timescale-$$" \
...
if docker run --rm \
    ...
    "${IMAGE}" \
    -h "postgres17-smoke-timescale-$$" -p 5432 -U postgres 2>&1 | grep -q 'accepting'; then
  _pass "postgres accepts connections (pg_isready from hardened image)"
else
  echo "[DEBUG] server logs:"; docker logs "postgres17-smoke-timescale-$$" 2>&1 | tail -20
...
# TimescaleDB installs a versioned filename (e.g. timescaledb-tsl-2.25.2.so)
