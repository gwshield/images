#!/usr/bin/env bash
# =============================================================================
# Smoke tests — gwshield-postgres v15.17-vector (vector search profile)
# Extensions tested: pg_partman, pg_cron, pgvector
# =============================================================================
set -euo pipefail
...
NET=$(docker network create "smoke-postgres-vector-$$" 2>/dev/null)
...
if docker run --rm \
    ...
    "${IMAGE}" \
    -h "postgres-smoke-vector-$$" -p 5432 -U postgres 2>&1 | grep -q 'accepting'; then
  _pass "postgres accepts connections (pg_isready from hardened image)"
else
  echo "[DEBUG] server container logs:"; docker logs "postgres-smoke-vector-$$" 2>&1 | tail -20
