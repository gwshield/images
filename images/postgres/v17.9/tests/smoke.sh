#!/usr/bin/env bash
# =============================================================================
# Smoke tests — gwshield-postgres v17.9 (standard profile)
...
NET=$(docker network create "smoke-postgres17-standard-$$" 2>/dev/null)
...
# Strategy: spin up postgres:17-alpine as the server (it handles initdb
...
CONTAINER=$(docker run -d \
    --network "${NET}" \
    --name "postgres17-smoke-standard-$$" \
    -e POSTGRES_PASSWORD=gwshield_test \
    -e POSTGRES_USER=postgres \
    postgres:17-alpine)

# Wait for postgres:17-alpine to finish initdb and start listening
sleep 8
...
if docker run --rm \
    ...
    "${IMAGE}" \
    -h "postgres17-smoke-standard-$$" -p 5432 -U postgres 2>&1 | grep -q 'accepting'; then
  _pass "postgres accepts connections (pg_isready from hardened image)"
else
  echo "[DEBUG] server container logs:"; docker logs "postgres17-smoke-standard-$$" 2>&1 | tail -20
